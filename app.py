...
# 5. UI 구성 - 상단 기상도 및 자산 비중
st.markdown(f"<div style='text-align: center; background-color: #f8f9fa; padding: 20px; border-radius: 20px; border: 1px solid #eee; margin-bottom: 25px;'><p style='font-size: 16px; color: #666; margin-bottom: 0;'>중기 포트폴리오 기상도</p><h1 style='font-size: 45px; color: {w_col}; margin: 0;'>{w_icon} {weather}</h1></div>", unsafe_allow_html=True)
...
def unified_card(col, title, val, sig, color, desc, url):
    col.markdown(f"""
        <a href="{url}" target="_blank" class="integrated-card" style="border-top: 8px solid {color};">
            <div style="font-size: 13px; color: #666; font-weight: bold; margin-bottom: 10px;">{title} 🔗</div>
            <div style="display: flex; justify-content: space-between; align-items: baseline; margin-bottom: 15px;">
                <span style="font-size: 24px; font-weight: 800; color: #222;">{val}</span>
                <span style="font-size: 16px; font-weight: bold; color: {color};">{sig}</span>
            </div>
            <div style="font-size: 14px; color: #444; line-height: 1.5; border-top: 1px solid #eee; padding-top: 10px;">{desc}</div>
        </a>
...
st.divider()
st.caption("※ 본 데이터는 투자 참고용이며 최종 판단은 투자자 본인에게 있습니다. 기본 주식 비중 25%를 기준으로 설계되었습니다.")
