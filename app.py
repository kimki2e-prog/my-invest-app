...
st.sidebar.markdown(f"🟡 금/원자재: **{round(m_gold)}** 만원")
st.sidebar.markdown(f"💵 현금: **{round(m_cash)}** 만원")
st.sidebar.info("아래 상세 ETF 종목 리스트의 개별 금액을 확인하세요!")
...
def asset_card(col, title, weight, color, amount):
    col.markdown(f"""
        <div style='background-color: {color}15; padding: 20px; border-radius: 15px; border: 2px solid {color}; text-align: center;'>
            ...
            <p style='color: #333; font-weight: bold; margin: 0;'>매수금액: {round(amount)}만원</p>
        </div>
    """, unsafe_allow_html=True)
...
# 9. 상세 ETF 포트폴리오 (개별 금액 자동 산출 버전)
st.subheader("📦 누구나 따라 할 수 있는 자산별 ETF 구성")
st.success(f"💡 **{input_money}만원** 기준, 종목별 추천 매수 금액입니다.")
col_st, col_bd, col_gd = st.columns(3)

with col_st:
    st.markdown("#### 📈 주식 (Growth)")
    st.markdown(f"""
    **🇰🇷 국내 주식 (50%) - 약 {round(m_stock*0.5)}만원**
    * Tiger 200 (20%): **{round(m_stock * 0.20)}** 만원
    * Rise 코리아밸류업 (15%): **{round(m_stock * 0.15)}** 만원
    * PLUS 고배당주 (15%): **{round(m_stock * 0.15)}** 만원
    
    **🇺🇸 미국 주식 (50%) - 약 {round(m_stock*0.5)}만원**
    * Tiger S&P500 (20%): **{round(m_stock * 0.20)}** 만원
    * Rise 미국나스닥 100 (10%): **{round(m_stock * 0.10)}** 만원
    * AI/인프라 테마 (각 10%): **{round(m_stock * 0.20)}** 만원
    """)

with col_bd:
    st.markdown("#### 🏦 채권 (Safety)")
    st.markdown(f"""
    **🇺🇸 미국 채권 (60%) - 약 {round(m_bond*0.6)}만원**
    * Kodex 미국30년국채액티브(H): **{round(m_bond * 0.30)}** 만원
    * ACE 미국국채10년액티브(H): **{round(m_bond * 0.30)}** 만원
    
    **🇰🇷 국내 채권 (40%) - 약 {round(m_bond*0.4)}만원**
...
with col_gd:
    ...
    st.markdown(f"""
    **설정 종목 (100%)**
    * ACE KRX금현물 ETF: **{round(m_gold)}** 만원
    """)
    st.markdown("#### 💵 현금 (Liquidity)")
    st.markdown(f"""
    **파킹형 ETF**
    * TIGER CD금리/KOFR: **{round(m_cash)}** 만원
    """)
